...
const DealerItem = ({item, nav}) => {
  ...
  return (
    <ListItem
      ...
      onPress={() => {navigation.navigate('ViewDealer', {user: item })
      }}
      keyExtractor={item => item.document_number}
    />
  );
};
const DealerItem2 = ({item, nav}) => {
  const navigation = nav;
  return (
    <ListItem
      roundAvatar
      ...
      .then(snapshot => {
        ...
        snapshot.forEach(child => {
          dealer_list.push({
            ...
            onpress: ()=>this.navigate('ViewUser', {user: child.val() })
          });
        });
          ...
          <FlatList
            ...
            renderItem={({item,navigate}) => {
              ...
              if (!item.verified == false) {
                console.log("true")
                return <DealerItem item={item, navigate} />;
              } else {
                console.log("false")
                return <DealerItem2 item={item,navigate} />;
              }
            }}
