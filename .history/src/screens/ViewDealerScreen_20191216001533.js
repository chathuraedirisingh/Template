...
export default class ViewDealerScreen extends Component {
  ...
  <View
    ...
    // ========================================
      ...
      return (
        ...
        <SafeAreaView style={{flex: 1}}>
          ...
          <DismissKeyboard>
          <KeyboardAvoidingView style={{ flex: 1, flexDirection: 'row',justifyContent: 'center'}} behavior="padding" keyboardVerticalOffset={150}>
    ...
    <View style={{flex:1,flexDirection:'row',justifyContent:'flex-start',margin:10}}>
      ...
      <View style={{opacity:0.6,flex:0,alignItems:'flex-start',marginRight:10}}>
        <Text>{`${this.state.data.name.first} ${this.state.data.name.last}`}</Text>
        <Text>
          {Moment(this.state.data.dob.date).format('DD/MM/YYYY')} |{' '}
          {this.state.data.phone}
